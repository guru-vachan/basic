...
if(isset($_POST['message']) && $_POST['message'] != '') {
	$sql = "INSERT INTO message(chat_id, user_id, user_name, message, post_time) VALUES (" . 
			db_input($_GET['chat']) . ",'$chat_to', '" . db_input($_POST['name']) . 
...
if(!isset($_GET['chat'])) {
	...
	$sql = "SELECT message_id, user_name,user_id, message, date_format(post_time, '%h:%i') as post_time" . 
		" FROM message WHERE user_id='$User_Name' && chat_id = " . db_input($_GET['chat']) . " AND message_id > " . $last;
	$sql1 = "SELECT message_id, user_name,user_id, message, date_format(post_time, '%h:%i') as post_time" . 
		" FROM message WHERE user_name='$User_Name'&& chat_id = " . db_input($_GET['chat']) . " AND message_id > " . $last;
	...
	$message_query1 = db_query($sql1);
	//Loop through each message and create an XML message node for each.
	while($message_array1 = db_fetch_array($message_query1)) {
		$xml .= '<message id="' . $message_array1['message_id'] . '">';
		$xml .= '<user>' . htmlspecialchars($message_array1['user_name']) . '</user>';
		$xml .= '<text>' . htmlspecialchars($message_array1['message']) . '</text>';
		$xml .= '<time>' . $message_array1['post_time'] . '</time>';
		$xml .= '</message>';
	}
}
$xml .= '</root>';
